...
## Быстрая проверка без API-ключа
...
Открыть http://127.0.0.1:3000. `check` проверяет форматирование, тесты, десять сценариев DoD, полный календарь, типы и production-сборку. Форма, готовые запросы и локальные объяснения работают без `.env.local`; данные и изображения уже в репозитории. Для AI-чата и голоса настройте собственный ключ по инструкции ниже.
...
```mermaid
flowchart LR
  A[Текст или голос] --> B[Быстрая модель: черновик]
  B --> C[Валидация и уточнение]
  D[Ручная форма] --> C
  C --> E[Подтверждение пользователя]
  E --> F[Код: фильтры и стабильный порядок]
  F --> G[До трёх анкет]
  G --> H[Выбор цитаты: nano или mini]
  H --> I[Проверка по источнику]
  I --> J[Карточки и объяснения]
  F --> K[Код: причины отказа и альтернативы]
  H -. Ошибка API .-> L[Локальные проверенные основания]
  G -. Без ключа или мало сведений .-> L
  L --> J
```
...
Единая команда полной локальной проверки: `npm run check`. Чистая копия устанавливается из lock-файла и работает без ключа команды; детали окружения и результаты — в [REPRODUCIBILITY.json](docs/REPRODUCIBILITY.json).
