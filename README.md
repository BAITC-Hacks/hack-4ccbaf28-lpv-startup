...
- Объяснение содержит дату, начальную цену, бюджет, формат и отдельное отличие из анкеты. Например, в сценарии приёмки это оборудование у Мицури, масштаб событий у Буллмы и профессиональный опыт Хаула. Цитата подтверждает наличие утверждения в источнике, а не его независимую проверку.
...
Полная сверка с ТЗ: [SPEC-AUDIT.md](docs/SPEC-AUDIT.md). Дополнительный независимый прогон `verify:spec` проверил **91 800 запросов по всем 100 датам**, городам, категориям и форматам каталога при трёх бюджетах. [Отчёт](docs/SPEC-CHECK-RESULTS.json).

Объяснения отбирают конкретные факты с приоритетом релевантности пожеланию. Метафоры не считаются опытом или оборудованием. Для семи описаний эвристика не находит достаточного конкретного отличия: интерфейс обозначает нехватку сведений, а не повторяет общую похвалу. `explanationEvidence.quality` показывает эту границу; это не экспертная оценка качества самого подрядчика. Причины пустой и короткой выдачи видны сразу.
