...
- тесты подбора и AI-интеграции, шесть сценариев приёмки, команды проверки типов, форматирования и сборки;
...
1. Нажмите готовый запрос для немедленного поиска по показанным условиям или откройте «Параметры» для своей заявки. С API-ключом можно описать событие в чате либо голосом. Новая заявка изначально пустая: неизвестные дата и бюджет не подставляются.
...
Подбор и отбор цитат — отдельные TypeScript-модули, не зависящие от интерфейса. `/api/search` возвращает также `explanationEvidence`: точную цитату, источник `description` и способ выбора `code` или `model`. В `availability.busyCandidates` перечислены ID и имена занятых на дату анкет нужной категории в городе; у них могут одновременно нарушаться и другие ограничения. Интерфейс показывает эти имена среди причин исключения.
...
- Для объяснений код отбирает точные фрагменты описания по конкретности деталей, отличиям от анкет той же категории в городе и совпадениям с пожеланиями. Если есть конкретные факты, общие похвалы не предлагаются модели. Модель получает основания и до четырёх вариантов цитат для каждого из максимум трёх кандидатов; её выбор должен точно совпасть с допустимым фрагментом, иначе остаётся локальное объяснение.
- Объяснение содержит дату, начальную цену, бюджет, формат и отдельное отличие из анкеты. Например, в основном демо это оборудование у Мицури, масштаб событий у Буллмы и профессиональный опыт Хаула. Цитата подтверждает наличие утверждения в источнике, а не его независимую проверку.
...
Готовые запросы «Ведущий на корпоратив», «Флорист на корпоратив» и «Ведущий до 100 тыс. ₸» показывают условия и **сразу запускают поиск по нажатию**. Дата берётся на момент загрузки по `Asia/Almaty`, пока она входит в учебный календарь; вне диапазона показывается явно подписанный пример с годом. Ограничения языка, часов и пожеланий в этих кнопках не заданы. В чате и при ручном редактировании заявки подбор по-прежнему требует подтверждения.

Для воспроизводимого сценария из отчёта задайте через «Параметры»: Алматы, Ведущий, корпоратив, **15.10.2026**, бюджет **1 000 000 ₸**, русский, 6 часов, пожелания «Интеллигентная, ненавязчивая подача и живой юмор». Нажмите «Подтвердить и подобрать». Следующая таблица использует эти фиксированные условия, а не сегодняшние быстрые запросы.
...
scripts/verify-dod.ts       — проверка шести сценариев приёмки
...
Проверено локально: установка из lock-файла, **27 из 27 тестов**, шесть сценариев `verify:dod`, типы, форматирование и production-сборка проходят. В браузере проверен немедленный поиск готового запроса по показанной дате. Календарь со сменой даты и сохранение избранного после перезагрузки проверены на предыдущем этапе интерфейса. Пустая выдача, редкая категория и отказ API также покрыты проверками доменного и серверного кода.
...
Быстрые запросы проверяются на смене суток по Алматы и за пределами учебного календаря: дата и бюджет явно показаны, скрытых ограничений языка, часов или пожеланий нет.
...
`npm test` использует имитацию ответов провайдера, а `npm run verify:dod` проверяет движок локально; обе команды не требуют ключа. В шесть сценариев входят массовая категория, повтор запроса, другая дата, редкая категория, отсутствие совпадений и отсутствие категории в городе. Проверяются порядок, строгие ограничения, источник и различимость цитат, причина исключения по дате и время каждого ответа менее 10 секунд.

Для проверки с реальными вызовами добавьте свой ключ в `.env.local` и выполните `npm run verify:dod:live`. Эта команда дополнительно требует AI-объяснение первого сценария и кешированный повтор; она расходует API. Отчёт можно сохранить флагом `--report <путь-к-json>` после `--`.

В [отчёте команды](docs/DOD-RESULTS.json) за 23.09.2026 шесть сценариев с реальным API прошли: максимум 3,541 с, повтор из кеша — 12 мс, суммарная оценка текстовых вызовов — около $0,00290. Этот результат не гарантирует такую же задержку и стоимость других запросов. Автоматические проверки подтверждают опору на источник; полезность объяснений для человека остаётся предметом ручной оценки. Критерии и сценарий защиты — в [DEFINITION-OF-DONE.md](docs/DEFINITION-OF-DONE.md).
