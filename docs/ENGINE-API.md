...
- For limited descriptions, optional `explanationEvidence[id].comparison` contains `text`, `source: "catalog_fields"`, and `peerIds` for the other displayed profiles. Code computes it without an LLM and also adds a `shortlist_comparison` item to the match evidence. It compares only the shortlist, never claims higher price means higher quality, and never interprets null attendance hours as unlimited work. Sole eligibility is stated only when `totalEligible` is one; tied fields produce no invented distinction.
