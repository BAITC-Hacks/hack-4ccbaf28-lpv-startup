...
- For limited descriptions, optional `explanationEvidence[id].comparison` contains a factual comparison, source `catalog_fields`, and the other displayed profile IDs. It compares only the shortlist, never claims higher price means higher quality, and never interprets null attendance hours as unlimited work. With one eligible profile it explains sole eligibility; tied fields produce no invented distinction.
