...
export function detailStrength(quote: string): number {
  ...
  let score = 0;
  if (
    /\d+\s*(?:человек|чел\b|гост|участник|мест|свад|камер|мероприяти|заказ|вокалист|музыкант|минут|час|дн)/.test(
      s,
    )
  )
    score += 7;
  // Concrete deliverable quantities, rather than an arbitrary number or future date.
  if (
    /\d+(?:-минутн|\s+(?:(?:обработанных|индивидуальных|коротк[а-я]*|командн[а-я]*|архивными|семейными|персональными|персональных|заранее записанными|согласованными|согласованных|графических|музыкальными|музыкальных|акустическими|протокольными|театральных|посадочных)\s+){0,3}(?:кадр|портрет|фотограф|вопрос|композици|песен|карточ|блок|раунд|истори|отбив|переход|сесси|сцен|сюжет|выход|ролей|пауз|видеовопрос|ролик|интервью|застав|номер|произведени|мест))/.test(
      s,
    )
  )
    score += 7;
  ...
  if (
    ...
    score += 6;
  if (
    /оборудован|мультимедиа|мультимедий|светов|звукорежиссер|\bdj\b|радиомикрофон|проектор|вспыш|радиопетличк|рекордер|стабилизатор|суфлер|кахон|домбр|фотозон|фотокниг|ретуш|дрон|аэросъем|арки|сезонн|живые цвет|инсталляц|террас|панорам|парков|банкетными зал|потолк|трансфер|банкетное меню|вегетариан|ручной работ|фотобудк|печать.{0,20}(?:снимк|фото)|брендирован|именные|логотип|леденцы|welcome-бокс/.test(
      s,
    )
  )
    score += 5;
